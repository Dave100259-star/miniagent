...
用法 (需真实 key):
    ...
    python eval/run_eval.py --ablation --repeat 3 --json eval/results.json
    python eval/run_eval.py --model glm-4-flash --max-steps 15
"""
...
def run_suite(llm_factory, tasks, max_steps, recover_errors, repeat, save_traces, label=""):
    ...
    print("=" * 66)
    per_task, tot_tok, tot_cost = [], 0, 0.0
    for task in tasks:
    ...
    avg_pass1 = sum(t["passes"] for t in per_task) / (len(per_task) * repeat)
    print("-" * 66)
    print(f"📊 pass@{repeat} = {pass_at_k:.0%}   平均单次通过率 = {avg_pass1:.0%}   "
          f"总 {tot_tok} tok   ${round(tot_cost, 5)}")
    return {"recover_errors": recover_errors, "repeat": repeat, "pass_at_k": pass_at_k,
            "avg_pass1": avg_pass1, "total_tokens": tot_tok,
            "total_cost": round(tot_cost, 6), "tasks": per_task}
...
def main():
    ...
    def llm_factory():
        return OpenAICompatLLM(model=args.model)
    ...
    if args.ablation:
        ...
        print(f"  平均单次通过率:   ON {on['avg_pass1']:>4.0%}   OFF {off['avg_pass1']:>4.0%}"
